...
package rs.math.oop1.zd.p060601.inicijalizacioniBlokPrimerka.z03.fakultet;
...
public class Student {
	...
	for (int i = 0; i < brojPredmeta; i++)
	    ocene[i] = -1;
	brojIndeksa = 2000+(new Random().nextInt(100));
	godinaUpisa = 2100; 
    }
    ...
    public void prikazi() {
	System.out.printf("\nStudent: %s %s, %d/%d\n", ime, prezime, brojIndeksa, godinaUpisa);
	System.out.printf("Smer: %s. Godina studija: %d\nOcene:\n", smer, godinaStudija);
	for (int i = 0; i < ocene.length; i++)
	    System.out.printf("%s:%s%s", Predmet.getSifraPredmeta(i),
		    (ocene[i] == -1) ? "<nema>" : String.valueOf(ocene[i]), (i == ocene.length - 1) ? "\n" : " ");
    }
	...
	if (pozicija != -1) {
	    stud.setBrojIndeksa(Integer.parseInt(brojIndeksa.substring(0, pozicija)));
	    stud.setGodinaUpisa(Integer.parseInt(brojIndeksa.substring(pozicija + 1)));
	}
	...
	System.out.printf("Unesi ocene iz %d predmeta (-1 znaci nije polagao)\n", Predmet.brojPredmeta);
	for (int i = 0; i < Predmet.brojPredmeta; i++) {
	    System.out.print(Predmet.getSifraPredmeta(i)+":");
