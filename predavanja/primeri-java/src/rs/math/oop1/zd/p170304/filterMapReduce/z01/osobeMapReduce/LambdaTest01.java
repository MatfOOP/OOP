package rs.math.oop1.zd.p170304.filterMapReduce.z01.osobeMapReduce;

import rs.math.oop1.zd.p170304.filterMapReduce.z01.osobeMapReduce.Person.Sex;
...
public class LambdaTest01
{
	...
	public static void main( String[] args )
	{
		Person.LJUDI.stream().filter( p -> {
			return p.gender == Sex.MALE && p.emailAddress.indexOf( "disney" ) >= 0;
		} ).forEach( p -> p.printPerson() );
		
		Person.LJUDI.stream().filter( x -> (x.gender == Sex.MALE) )
				.map( y -> y.name.length() )
		...
		long sumaGodina = Person.LJUDI.stream().filter( p -> {
			return p.gender == Sex.MALE && p.emailAddress.indexOf( "wb" ) >= 0;
		...
		double proizvodGodina2 = Person.LJUDI.stream()
				.filter(
						p -> p.gender == Sex.MALE && p.emailAddress.indexOf( "wb" ) >= 0 )
